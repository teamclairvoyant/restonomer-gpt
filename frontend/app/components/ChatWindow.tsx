...
export function ChatWindow(props: {
  ...
  const sendMessage = async (message?: string) => {
    ...
    renderer.code = function (code, language) {
      ...
      return `<pre class="highlight bg-gray-700" style="padding: 5px; border-radius: 5px; overflow: auto; overflow-wrap: anywhere; white-space: pre-wrap; max-width: 100%; display: block; line-height: 1.2"><code class="${language}" style="color: #d6e2ef; font-size: 14px; ">${highlightedCode}</code></pre>`;
    };
    ...
    >
      {messages.length > 0 && (
        <Flex direction={"column"} alignItems={"center"} paddingBottom={"20px"}>
          <Heading fontSize="2xl" fontWeight={"medium"} mb={1} color={"green.700"}>{titleText}</Heading>
          <Heading fontSize="md" fontWeight={"normal"} mb={1} color={"grey"}>
      ...
      <InputGroup size='md' alignItems={"center"} >
        ...
        value={input}
          ...
          rounded={"full"}
          type={'text'}
          placeholder='What is the use of Restonomer ?'
          textColor={"black"}
